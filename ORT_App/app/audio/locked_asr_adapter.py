...
class LockedASRProviderAdapter:
    """Exact provider adapter used by v9.0.4 model-lock sessions.
    ...
    def _load_reazon(self) -> None:
        ...
        self.model = load_model(device=self.device, precision=precision, language="ja")
        self.compute_type = precision
        self.model_path = "huggingface-cache:reazon-research/reazonspeech-k2-v2"
        self._backend = "reazonspeech_k2"

    def _load_sensevoice(self) -> None:
        if self.device != "cpu":
            raise RuntimeError("SenseVoice-Small sherpa-onnx adapter is CPU-only in ORT v9.0.4.")
        import sherpa_onnx
    ...
    def _recognize(self, audio: np.ndarray, stable: bool) -> tuple[str, dict]:
        ...
        if self._backend == "reazonspeech_k2":
            ...
            result = transcribe(self.model, audio_from_numpy(np.asarray(audio, dtype=np.float32), TARGET_SAMPLE_RATE))
