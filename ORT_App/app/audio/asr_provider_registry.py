...
def provider_status(model_root: Path, provider_id: str, device: str = "cpu", runtime_python: Path | None = None) -> dict:
    ...
    if provider_id in {PROVIDER_KOTOBA, PROVIDER_WHISPER_BASE, PROVIDER_WHISPER_SMALL}:
        ...
        location = Path(str(marker.get("model_path") or os.environ.get("HF_HOME") or Path.home() / ".cache" / "huggingface"))
        warnings.append("Weight Reazon dipakai bersama CPU/GPU; kesiapan runtime tetap diperiksa per device.")
