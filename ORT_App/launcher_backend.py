...
def project_root_path() -> Path:
    """Resolve both legacy <root>/ORT/runtime_app and v9 <root>/ORT_App."""
    if BASE_DIR.name.lower() == "runtime_app" and BASE_DIR.parent.name.lower() == "ort":
...
def get_runtime_config():
    ...
    runtime_root = cfg.get("runtime_root") or str(PROJECT_ROOT / "ORT_Runtime")
    project_root = cfg.get("project_root") or str(PROJECT_ROOT)
    runtime_python = str(Path(runtime_root) / ".venv" / "Scripts" / "python.exe")
    valid = Path(runtime_python).exists()
    storage_mode = "surface" if Path(runtime_root).resolve() == (PROJECT_ROOT / "ORT_Runtime").resolve() else "custom"
    return {
