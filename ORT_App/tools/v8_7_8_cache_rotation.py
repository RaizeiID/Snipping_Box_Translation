"""Dry-run/apply semantic cache rotation for ORT v8.7.8 Faithfulness v2."""
from __future__ import annotations
import argparse, re, shutil, time
from pathlib import Path
NEW_NAMESPACE="v8_7_8_faithfulness_v2_strict_ct2_safe"
OLD_NAMESPACES=("v8_7_2","v8_7_3_identity_guard","v8_7_4_entity_span_responsive","v8_7_5_verified_exact_entity_safe","v8_7_6_adaptive_ocr_exact_safe","v8_7_7_faithful_complete_ct2_safe")
...
def run(base_dir='.', apply=False):
    ...
    lines=['ORT Translation v8.7.8 Semantic Cache Rotation '+('APPLY' if apply else 'DRY-RUN'), f'Target namespace: {NEW_NAMESPACE}', f'Candidates: {len(rows)}']
    ...
    if not apply:
        ...
        return '\n'.join(lines)
    backup=base/'backups'/f'cache_before_v8_7_8_faithfulness_v2_{time.strftime("%Y%m%d_%H%M%S")}'
