"""Dry-run/apply migration helper for v8.7.4 identity registry and contaminated cache."""
...
CONTAMINATED = ('Phaedusa','Balthalde','Helena Helen','Helena I Helen','Helena Il Helen','ORT_ENTITY','_ _ ORT')
...
def run(base_dir=None, apply=False):
    ...
    lines=['ORT v8.7.4 Identity/Data Migration ' + ('APPLY' if apply else 'DRY-RUN'),
    ...
    if apply and cache_hits:
        backup=base/'backups'/f'cache_before_v8_7_4_entity_span_responsive_{time.strftime("%Y%m%d_%H%M%S")}'
