"""Create v8.7.9 safety/test ledger from recent session event logs.
...
def build(base_dir: str | Path = ".", *, max_files: int = 30, max_lines_per_file: int = 500_000) -> dict:
    ...
    return {"version": "v8.7.9", "generated_at": time.time(), "policy": "Recent session ledger; increase --max-files only for archival audit.", "max_files": max_files, "max_lines_per_file": max_lines_per_file, "session_count": len(rows), "sessions": rows}
...
def main() -> int:
    ...
    out = base / "logs" / f"v8_7_9_test_ledger_{time.strftime('%Y%m%d_%H%M%S')}.json"
