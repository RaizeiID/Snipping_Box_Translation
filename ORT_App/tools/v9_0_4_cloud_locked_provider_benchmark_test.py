...
def main() -> int:
    ...
    for marker in (
        ...
        "Siapkan model yang dipilih",
        "Target setup/download · wajib dipilih setiap kali",
    ...
    assert '"--device", "cuda" if device == "cuda" else "cpu"' in webui
    assert "Runtime GPU masih build CPU" in (APP_ROOT / "app" / "audio" / "asr_provider_registry.py").read_text(encoding="utf-8-sig")
