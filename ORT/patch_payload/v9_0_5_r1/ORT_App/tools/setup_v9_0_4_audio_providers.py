...
SETUP_RELEASE = "Engineering Baseline & Resilient Provider Setup"
SETUP_USER_AGENT = "ORT-v9.0.5-r1-provider-setup/1.1"
...
def _safe_error_summary(exc: BaseException, limit: int = 900) -> str:
    text = _exception_chain_text(exc).replace("\r", " ").replace("\n", " ")
...
def _transient_download_error(exc: BaseException) -> bool:
    ...
    needles = (
        "connectionreseterror", "winerror 10054", "winerror 10053",
        "remoteprotocolerror", "server disconnected", "connection aborted",
        "connection reset", "timed out", "timeout", "temporarily unavailable",
        "502", "503", "504", "rate limit", "429", "incompletesnapshoterror",
        "connecterror", "urlerror", "remotedisconnected", "sslerror",
        "connection refused", "network is unreachable", "name resolution",
        "temporary failure in name resolution", "connection closed",
        "forcibly closed by the remote host", "dryrunerror",
    )
    if any(token in text for token in needles):
        # DryRunError by itself can also indicate a private/missing repository.
        # Treat it as transient only when the nested chain contains a network
        # exception or an explicit connectivity phrase.
        if "dryrunerror" in text and len(chain) == 1:
            return any(token in text for token in (
                "internet connection", "connection", "timeout", "network", "temporarily"
            ))
        return True
    for item in chain:
        ...
        if errno in {54, 10053, 10054, 10060, 11001}:
            return True
    return False
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            emit(
                ...
                root_cause=f"{type(_root_cause(exc)).__name__}: {_root_cause(exc)}",
            )
            print(
                f"[DOWNLOAD RETRY] {label} gagal ({attempt}/{max_attempts}): "
                f"{_safe_error_summary(exc)}. Mencoba lagi dalam {delay:.1f}s.",
                flush=True,
            )
            time.sleep(delay)
        ...
        try:
            ...
            if exc.code == 416 and offset == expected_size:
                if expected_hash and _sha256_file(partial).lower() != expected_hash:
                    raise IOError(f"Checksum SHA-256 gagal untuk partial {filename}") from exc
...
def _reazon_local_warmup_script(model_dir: Path, device: str) -> str:
    ...
    return (
        ...
        "from pathlib import Path; "
        "from app.audio.sherpa_compat import create_offline_transducer; "
        f"p=Path({str(model_dir)!r}); "
        "m,info=create_offline_transducer("
        ...
        f"print('ReazonSpeech K2 local {target.upper()} model ready | '+json.dumps(info.as_dict(), ensure_ascii=False))"
    )


def _download_url(provider: str, url: str, destination: Path) -> tuple[int, int]:
    destination.parent.mkdir(parents=True, exist_ok=True)
    request = urllib.request.Request(url, headers={"User-Agent": SETUP_USER_AGENT})
...
def self_test() -> int:
    ...
    assert _transient_download_error(outer)
    assert "WinError 10054" in _exception_chain_text(outer) or "10054" in _exception_chain_text(outer)
    ...
    print(json.dumps({
        "passed": True, "version": SETUP_VERSION,
...
def _write_failure_report(
    ...
    status_root.mkdir(parents=True, exist_ok=True)
    safe_provider = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in str(provider))
    safe_device = "cuda" if str(device).lower() == "cuda" else "cpu"
    path = status_root / f"last_error_{safe_provider}_{safe_device}.log"
    payload = [
        f"ORT provider setup {SETUP_VERSION}",
        f"Release: {SETUP_RELEASE}",
        f"Provider: {provider}",
        f"Device: {device}",
        f"Transient network error: {_transient_download_error(exc)}",
        f"Exception chain: {_safe_error_summary(exc, 4000)}",
        "",
...
def main() -> int:
    ...
    try:
        ...
        if provider in {"sensevoice_small", "all"}:
            if args.device == "cuda":
                raise RuntimeError("SenseVoice-Small GPU belum didukung pada v9.0.5")
        ...
        log_path = _write_failure_report(status_root, provider, args.device, exc)
        transient = _transient_download_error(exc)
        root = _root_cause(exc)
        recovery = (
            "File yang sudah selesai dan file .part tetap dipertahankan. "
            "Jalankan target yang sama untuk melanjutkan."
        )
        emit(
            "setup_failed", provider=provider, device=args.device, passed=False,
            error=_safe_error_summary(exc),
        ...
        print(f"[SETUP ERROR] {_safe_error_summary(exc)}", flush=True)
        print(f"[DIAGNOSTIK] {log_path}", flush=True)
        print(f"[PEMULIHAN] {recovery}", flush=True)
        if os.environ.get("ORT_SETUP_DEBUG", "").strip() == "1":
            import traceback
            traceback.print_exc()
        return 1
