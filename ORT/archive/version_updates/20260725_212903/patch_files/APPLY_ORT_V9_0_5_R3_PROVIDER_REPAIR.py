...
PATCH_NAME = "ORT v9.0.5 R3 Offline Argos & Native CUDA DLL Repair"
...
def main() -> int:
    ...
    backup = root / "ORT" / "backups" / f"ORT_V9_0_5_R3_PROVIDER_REPAIR_{stamp}"
    receipt = root / "ORT" / "status" / "v9_0_5_r3_provider_repair_receipt.json"
    copied: list[str] = []
    created: list[str] = []
    ...
    try:
        ...
        print("ORT_V9_0_5_R3_PROVIDER_REPAIR: PASS")
        print(f"Receipt: {receipt}")
        return 0
    except Exception as exc:
        print(f"ORT_V9_0_5_R3_PROVIDER_REPAIR: FAIL · {type(exc).__name__}: {exc}", file=sys.stderr)
