"""Dry-run/apply v8.7.8 identity, term and observed-ledger alignment safely."""
...
def run(base_dir='.', apply=False):
    ...
    ledger=load(configs/'gfl2_observed_candidates_v8_7_8.json', {})
    ...
    additions=[clean(x.get('display_name','')) for x in ledger.get('approved_exact_additions_v8_7_8',[]) if isinstance(x,dict)]
    ...
    lines=['ORT Translation v8.7.8 Identity & Terms Migration '+('APPLY' if apply else 'DRY-RUN'), 'Policy: official catalog exact-only; fresh verified names/roles exact-only; aliases ROI-only; Commander names never global.', f'Official exact-speaker entries: {len(official)}', f'Official removed from legacy_untrusted: {len(promoted)}', f'Approved exact/role available: {", ".join(approved_names) or "-"}', f'Fresh v8.7.8 additions: {", ".join(additions) or "-"}', f'Special terms total from defaults: {len(terms)}', 'Excluded global commander names: Vilyz, ARVITA ID, ATVITA ID', 'Pending review not auto-live: Chief of Odesa, Municipal Broadcast, Zyevnadya/Yyevnadya']
    if not apply:
        lines.append('Gunakan --apply pada folder TEST setelah backup. Candidate mining tidak pernah auto-promote.')
        return '\n'.join(lines)
    backup=base/'backups'/f'identity_before_v8_7_8_{time.strftime("%Y%m%d_%H%M%S")}'; backup.mkdir(parents=True,exist_ok=True)
    ...
    g['pending_session_candidate']=sorted(set(g.get('pending_session_candidate',[])+['Chief of Odesa','Municipal Broadcast','Zyevnadya / Yyevnadya']),key=str.casefold)
    registry['schema_version']='v8_7_8_faithfulness_v2_strict_ct2_registry_v6'
    registry.setdefault('migration_notes',[]).append({'version':'v8.7.8','ts':time.time(),'change':'Fresh exact names/roles + observed ledger + terms; pending ambiguous candidates; commander excluded globally'})
