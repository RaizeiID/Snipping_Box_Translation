...
def render_observed_review_html(game: str) -> str:
    """Render v8.8.3 observed speaker/term/alias review without auto-activating aliases."""
    ...
    return (
        ...
        "<hr><b>v8.8.3 Safe Additions — exact-only setelah migrasi</b><p class='smallnote'>Nama biru berasal dari log CT2 terbaru dan tidak memakai fuzzy body matching.</p>" + add_chips +
        "<hr><b>Special Terms GFL2 v8.8.3 baru/retained</b><div>" + term_chips + "</div>" +
...
with gr.Blocks(title="ORT Translation v8.8.3") as demo:
    gr.HTML("<div class='hero'><h1>ORT Translation v8.8.3</h1><p>GFL2 Recording Stability, Overlay Commit Gate, CT2 Path Resolver, Render Signature, dan Recording Telemetry.</p></div>")
    with gr.Row():
    ...
    with gr.Tabs():
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    ...
                    with gr.Group(visible=_INITIAL_VIS["model_controls"]) as model_controls_panel:
                        ...
                        gr.HTML("<div class='smallnote'>v8.8.3: perubahan Mode / Engine / Interval / OCR disimpan otomatis per model; override OCR manual tidak diturunkan diam-diam. Badge <b style='color:#fb923c'>• Modification</b> muncul jika model sudah berbeda dari default bawaan.</div>")
                        ...
                        with gr.Row():
                            ...
                            gr.Markdown("Mode responsif memprioritaskan dialog terbaru dan mengurangi preview usang. Two-Pass Name ROI tetap aktif; v8.8.3 dapat menaikkan OCR sementara bila teks preset rendah rusak.")
        ...
        with gr.Tab("Pengolahan Data"):
            ...
            with gr.Group(visible=not bool(SETTINGS.get("simple_view_enabled", True))) as identity_normal_group:
                ...
                gr.Markdown("### Observed Story / Alias Review v8.8.3")
