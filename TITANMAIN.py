...
VERSION = "TITAN X v6.3+ - TITANMAIN (CAS FAST + Shift+F9 + CAS Progress)"
...
def _apply_webui_boot_preset():
    ...
    if ORT_BOOT_MODE or ORT_BOOT_ENGINE:
        print(f"[BOOT] WebUI preset | mode={ORT_BOOT_MODE or '-'} | engine={ORT_BOOT_ENGINE or '-'} | auto_ms={AUTO_SNAPSHOT_INTERVAL_MS}ms")

_apply_webui_boot_preset()


# VRAM monitor thresholds (AUTO_GPU only)
VRAM_LOW_GB = float(os.environ.get("TITAN_VRAM_LOW_GB", "0.8"))
VRAM_RECOVER_GB = float(os.environ.get("TITAN_VRAM_RECOVER_GB", "1.4"))
VRAM_CHECK_INTERVAL_MS = int(os.environ.get("TITAN_VRAM_CHECK_MS", "2000"))
VRAM_COOLDOWN_SEC = float(os.environ.get("TITAN_VRAM_COOLDOWN_SEC", "8.0"))
...
SPEAKER_PROMOTE_HITS = int(os.environ.get("TITAN_SPEAKER_PROMOTE_HITS", "2"))
...
def save_cache(reason="manual"):
    ...
    log(f"[CACHE] Saved ({reason}) | mem={len(TRANSLATION_MEMORY)} npc={len(NPC_DATABASE.get('known', []))} unique={len(UNIQUE_TERMS.get('known', []))}")
...
def offline_translate_ram(text: str) -> str:
    # selalu output ke Bahasa Indonesia (id)
    if not text:
        return ""
    ...
    try:
        ...
        if has_cjk:
            # prefer zh->id direct, else zh->en->id
            try:
                out = _argos_translate(text, "zh", "id")
            except Exception:
                temp = _argos_translate(text, "zh", "en")
                out = _argos_translate(temp, "en", "id")
        else:
            out = _argos_translate(text, "en", "id")
...
def _strip_ocr_digit_noise(s: str) -> str:
    if not s:
        return s
    s = re.sub(r"^\s*(?:\d+\]|\d+\)|\d+\||\(\@|\@\)|\[\d+\]|\d+\s+)", "", s).strip()
    s = re.sub(r"(?<![\d\/\.\-%])\b([0-9])\b(?![\d\/\.\-%])", "", s)
...
def _learn_speaker(name: str):
    ...
    if len(name) < 2:
        return
    if _is_unique_like(name):
        return
    if _looks_noise_token(name):
        return
...
def _vram_monitor_tick():
    global _last_vram_switch_ts, CURRENT_ENGINE_MODE, AUTO_GPU_FALLBACK_CPU
    try:
        ...
        if (not AUTO_GPU_FALLBACK_CPU) and IS_USING_GPU and free_gb < VRAM_LOW_GB:
            log(f"[VRAM] Low VRAM: {free_gb:.2f}GB/{total_gb:.2f}GB -> AUTO fallback CPU")
...
class OCRWorker(QThread):
    ...
    def _prep_stable(self, img_rgb):
        scale = 1.10 if IS_USING_GPU else 1.05
        up = cv2.resize(img_rgb, None, fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR)
        gray = cv2.cvtColor(up, cv2.COLOR_RGB2GRAY)
        return gray

    def _prep_freeze_variants(self, img_rgb):
        base_scale = 1.20 if IS_USING_GPU else 1.12
        up = cv2.resize(img_rgb, None, fx=base_scale, fy=base_scale, interpolation=cv2.INTER_LINEAR)
        gray = cv2.cvtColor(up, cv2.COLOR_RGB2GRAY)
    ...
    def _queue_push(self, mode_name: str, text: str):
        text = (text or "").strip()
        if len(text) < 2:
            return False
        ...
        try:
            OCR_TEXT_QUEUE.put_nowait((mode_name, text))
    ...
    def _readtext_fast(self, img_gray):
        with OCR_LOCK:
                ...
                try:
                    ...
                    if CAPTURE_MODE == 1:
                        ...
                        if snap_kind is None:
                            self.msleep(18 if IS_USING_GPU else 40)
                            continue
                    ...
                    if CAPTURE_MODE == 0:
                        self._queue_push("STABLE", raw_text)
                    elif CAPTURE_MODE == 2:
                        if len(raw_text) > 2:
                            ...
                            if len(self.vote_buf) >= 5:
                                ...
                                if most[1] >= 3:
                                    self._queue_push("HIGH_LATENCY", most[0])

                except Exception as e:
                    self.debug.emit(f"[OCR] ERR: {e}")

                self.msleep(18 if IS_USING_GPU else 45)
...
class TranslatorWorker(QThread):
    ...
    def run(self):
        while self.running:
            try:
                mode_name, raw_text = OCR_TEXT_QUEUE.get(timeout=0.25)
            except queue.Empty:
                continue
            ...
            self.debug.emit(f"[PIPE] cache={'HIT' if cache_hit else 'MISS'} | {dt}ms | mode={mode_name} | kind={kind}")
            training_log_record(mode_name, dialog, out, meta={"speaker": speaker, "kind": kind})
...
def boot_system():
    ...
    print(" [F7]  FONT SIZE (+)")
    print(" [F8]  BOX WIDTH")
    print(" [F9]  OCR ENGINE: AUTO_GPU -> FORCE_GPU -> FORCE_CPU")
...
def main():
    import atexit
    atexit.register(lambda: save_cache("atexit"))
